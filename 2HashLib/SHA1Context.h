...
public:
	...
	SHA1Context(const SHA1Context &orig);
	SHA1Context& operator=(const SHA1Context &orig) = delete;
	SHA1Context(SHA1Context &&orig);
	SHA1Context& operator=(SHA1Context &&orig) = delete;
...
private:
	static const unsigned int hashSize = 20U;
	...
	*/
	std::vector<unsigned char> * sha1Ctx = nullptr;
};
